(function () {
  ...
  class HardcoverWidget {
    ...
    async loadBooks() {
      if (!this.config.username) {
        this.showError(
          "Username is required. Please add data-username attribute.",
        );
        return;
      }
      ...
      try {
        const endpoint =
          this.config.bookType === "last-read"
        ...
        this.element.dispatchEvent(
          new CustomEvent("hardcover:loaded", {
        ...
        this.element.dispatchEvent(
          new CustomEvent("hardcover:error", {
    ...
    showEmptyState() {
      this.element.innerHTML =
        '<div class="hw-empty-state">No books currently being read</div>';
    }

    renderBooks(books) {
      // Apply limit if specified
      const displayBooks =
        this.config.limit && this.config.limit > 0
          ? books.slice(0, this.config.limit)
          : books;

      const booksHtml = displayBooks
    ...
    renderBook(book) {
      const cover =
        book.book.image && book.book.image.url
  ...
  function autoInit() {
    ...
    widgets.forEach((element) => {
      ...
      if (element.dataset.minColumnWidth)
        config.minColumnWidth = element.dataset.minColumnWidth;
      ...
      if (element.dataset.limit)
        config.limit = parseInt(element.dataset.limit, 10);
